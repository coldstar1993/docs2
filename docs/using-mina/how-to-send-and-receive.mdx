...
# How to Send & Receive 怎么发送和接收MINA

<Subhead>
Learn how to send and receive MINA 学习怎么发送和接收MINA
</Subhead>
...
首先，确保你[安装了Mina 钱包](../using-mina/install-a-wallet)。

### Receiving MINA 接收MINA

To receive MINA, you must provide the unique address for your Mina account to the sender.
为了接收MINA, 你必须提供你Mina账户的唯一地址给发送者。

Here’s how to find your address within common wallets:下面是如何在各种钱包中找到你的地址:
...
当发送MINA时，务必重复检查你输入的是接收者的正确Mina地址。记住: <i>Mina链上的交易是终结的和不可逆的</i>
...
当在区块链(例如Mina链)上发送交易时, 发送者必须加上交易费用。大多数钱包会计算一个推荐费用。你也可以到这个社区创建的[Gas Station website](https://fees.mina.tools/)查看基于当前网络交易量的推荐费用。
:::

### Viewing your transction on a blockchain explorer 在区块链浏览器上查看你的交易

You can view your transactions using one of the community-run blockchain explorers:
你可以在社区运行的区块链浏览器上查看你的交易：
